...
// SAME AS YOUR TEST SCRIPT
export function isDeliveredFactured(status) {
  const s = normalizeStatus(status);
  return s.includes("livre") && s.includes("facture");
}

// SAME AS YOUR TEST SCRIPT
export function isFailedOrReturned(status) {
  const s = normalizeStatus(status);
...
function isReportIgnored(status) {
  ...
  // SHExpress dashboard does not count "Ajouté" in Total for this report
  return s === "ajoute";
}
...
function isDashboardFailed(status) {
  ...
  // Match SHExpress dashboard "Echoués"
  return s === "annule" || s === "annulee" || s === "retour client expedie";
}
...
export async function getShipmentsStatsByDateRange(fromDate, toDate) {
  ...
  const failedRows = reportRows.filter((r) => isDashboardFailed(r.status));

  const inProgressRows = reportRows.filter(
    (r) => !isDelivered(r.status) && !isDashboardFailed(r.status),
  );
  ...
  return {
    ...
    delivered: deliveredRows.length,
    delivered_factured: deliveredFacturedRows.length,
    delivered_not_factured: deliveredNotFacturedRows.length,
    in_progress: inProgressRows.length,
    ...
    // Keep raw status counts for debugging
    status_counts: buildStatusCounts(rows),

    // Keep all rows in data, so delivery table still shows everything
    data: rows,
  };
}
