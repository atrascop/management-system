...
export async function autoSyncMetaAdsForRange(filters = {}) {
  const today = new Date().toISOString().slice(0, 10);
  ...
  if (autoSyncLocks.has(lockKey)) {
    console.log("⏭️ Meta Ads sync already running:", lockKey);
    return autoSyncLocks.get(lockKey);
  }

  const promise = syncMetaAdsForRange({ from, to })
    .catch((error) => {
      console.error("❌ Auto Meta Ads sync failed:", error.message);
  ...
  autoSyncLocks.set(lockKey, promise);
